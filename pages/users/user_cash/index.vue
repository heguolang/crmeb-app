<template>
	<view :data-theme="theme">
		<view class='cash-withdrawal'>
			...
			<view class='panel'>
				<view class='nav acea-row' v-if="navList.length">
					<view v-for="(item,index) in navList" :key="item.type" class='item' :class="currentTab==index ? 'on':''" @click="swichNav(index)">
						<view class='iconfont' :class='item.icon'></view>
						<view class="tab_text">{{item.name}}</view>
					</view>
				</view>
				<view class='tip empty-type' v-else>暂无可用提现方式，请联系管理员配置</view>
				<view class='wrapper' v-if="navList.length">
					<view v-if="currentType === 'bank'" class='list'>
					...
					<view v-if="currentType === 'weixin'" class='list'>
						<form @submit="subCash" report-submit='true'>
							<view class='item acea-row row-between-wrapper'>
					...
					<view v-if="currentType === 'alipay'" class='list'>
						<form @submit="subCash" report-submit='true'>
							<view class='item acea-row row-between-wrapper'>
	...
	export default {
		data() {
			return {
				...
				enabledExtractTypes: ['bank', 'weixin', 'alipay'],
				currentTab: 0,
				index: 0,
				array: [],
				minPrice: 0.00,
		...
		computed: {
			...mapGetters(['isLogin']),
			navList() {
				return this.allNavList.filter((item) => this.enabledExtractTypes.indexOf(item.type) > -1);
			},
			currentType() {
				const item = this.navList[this.currentTab];
		...
		methods: {
			...
			getExtractUser(){
				extractUser(this.extractSource).then(res=>{
					...
					const types = String(res.data.extractType || 'bank,weixin,alipay')
						.split(',')
						.map((s) => (s || '').trim())
						.filter((s) => ['bank', 'weixin', 'alipay'].indexOf(s) > -1);
					this.enabledExtractTypes = types.length ? types : ['bank', 'weixin', 'alipay'];
					if (this.currentTab >= this.navList.length) {
						this.currentTab = 0;
					}
				})
			},
			...
			subCash: Debounce(function(e) {
				let that = this,
					...
					if (!that.currentType) {
						return this.$util.Tips({ title: '暂无可用提现方式' });
					}
					if (that.currentType === 'bank') {
					...
					} else if (that.currentType === 'weixin') {
					...
					} else if (that.currentType === 'alipay') {
...
<style lang="scss">
	...
	.cash-withdrawal .nav .item .tab_text {
		line-height: 1.2;
	}

	.cash-withdrawal .nav .item.on {
		color: var(--view-theme);
	}

	.cash-withdrawal .nav .item.on .iconfont {
		color: var(--view-theme);
	}

	.cash-withdrawal .nav .item.on::after {
		content: '';
		position: absolute;
		left: 50%;
		bottom: -24rpx;
	...
	.cash-withdrawal .wrapper .list .bnt[disabled] {
		opacity: 0.5;
	}
